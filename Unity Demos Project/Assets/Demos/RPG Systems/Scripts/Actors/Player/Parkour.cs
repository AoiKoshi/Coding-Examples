...
public class Parkour : ControllerModule
{
    ...
    {
        if (controller.currentState == Controller.characterStates.active && controller.readInput.sprintHeld)
        {
            if (VaultCheck())
            {
    ...
    private void Vaulting()
    {
        if (controller.currentState == Controller.characterStates.vaulting)
        {
            ...
            {
                transform.position = Vector3.Lerp(transform.position, vaultPeak, vaultDelay * 1f / vaultDelay);
